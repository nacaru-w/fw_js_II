const findOne = (list, { key, value }, { onSuccess, onError }) => { // Declara una variable findOne que contiene una arrow function que posee como tres argumentos: una variable (por el nombre parece ser un array), un objeto con dos elementos key y value, y otro con dos elementos, onSuccess y onError.
  setTimeout(() => { // La función anterior llama a otra función global setTimeout con dos argumentos: el primero es una arrow function completa y el segundo es el número 2000. Esto hará que la arrow function solo se ejecute después de haber pasado 2000 milisegundos (2 segundos) después de ser llamada.
    ...
    element ? onSuccess(element) : onError({ msg: 'ERROR: Element Not Found' }); // Aquí se usa un optional chaining operator para evaluar el resultado de la función de la línea anterior. Si esta arroja un dato llamará a la función onSuccess, a la cual le pasa el propio dato como argumento. De lo contrario, llamará a la función onError, a la cual le pasará como argumento un par key/value 
  }, 2000); // Como ya se ha descrito más arriba, aquí se especifica el tiempo tras el cual se ejecutará la función en milisegundos
};

const onSuccess = ({ name }) => console.log(`user: ${name}`); // Aquí se declara la función onSuccess, que toma como argumento un nombre y lo imprime por consola junto con el resto de lo especificado en el template literal
const onError = ({ msg }) => console.log(msg); // Aquí se declara la función onError, que toma como argumento el mensaje de error pasado más arriba si el método .find no devuelve un dato de la lista
